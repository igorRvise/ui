  ...
  return (
    <>
      ...
      <Group mt={40}>
        ...
        <Button variant="outline" color="red" onClick={() => setCount((c) => (c - 1 > 0 ? c - 1 : 0))}>
          Remove step
        </Button>
...
const sizes = RVISE_SIZES.map((size, index) => <Wrapper key={size} size={size} mt={index !== 0 ? 60 : 0} />)

const verticalSizes = RVISE_SIZES.map((size, index) => (
