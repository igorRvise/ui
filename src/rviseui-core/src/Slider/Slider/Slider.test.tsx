...
const expectInputValue = (value: string, container: HTMLElement) => expect(getInput(container)).toHaveValue(value)

const pressArrow = (kind: 'right' | 'left') =>
  userEvent.type(screen.getByRole('slider'), kind === 'right' ? '{arrowright}' : '{arrowleft}')

describe('@rviseui/core/Slider', () => {
  checkAccessibility([<Slider {...defaultProps} />])
  itSupportsSystemProps({
    component: Slider,
    props: defaultProps,
    displayName: '@rviseui/core/Slider',
