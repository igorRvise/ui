...
import { RVISE_COLORS, useRviseTheme } from '@rviseui/styles'
...
export function Colors() {
  const items = RVISE_COLORS.map((color) => (
...
export function States() {
  ...
  const sharedStyles: CSSProperties = {
    padding: '10px 20px',
    border: `1px solid ${theme.colorScheme === 'light' ? theme.colors.gray[1] : theme.colors.dark[6]}`,
  }
...
export function CustomComponent() {
  return (
    <div style={{ padding: 40 }}>
      <Button component="a" href="https://rvise.dev">
        Anchor
      </Button>
    </div>
  )
