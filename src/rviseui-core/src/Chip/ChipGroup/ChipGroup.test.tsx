...
const clickChip = (index: number, type: 'radio' | 'checkbox') => userEvent.click(getChip(index, type))

describe('@rviseui/core/ChipGroup', () => {
  checkAccessibility([<ChipGroup {...defaultProps} />])
  itSupportsSystemProps({
    component: ChipGroup,
    props: defaultProps,
    displayName: '@rviseui/core/ChipGroup',
  ...
  it('supports uncontrolled state when multiple prop is false', async () => {
    ...
    expect(screen.getAllByRole('radio').every((chip) => chip.getAttribute('checked') === null)).toBe(true)
  ...
  it('supports uncontrolled state when multiple prop is true', async () => {
    ...
    expect(screen.getAllByRole('checkbox').every((chip) => chip.getAttribute('checked') === null)).toBe(true)
