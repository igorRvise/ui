...
import { itRendersChildren, checkAccessibility, wait, renderWithAct } from '@rviseui/tests'
...
describe('@rviseui/core/component', () => {
  ...
  it('correctly handles withArrow={true}', () => {
    ...
    expect(container.querySelectorAll('.rviseui-Popover-arrow')).toHaveLength(1)
  })

  it('correctly handles withArrow={false}', () => {
    const { container } = render(<TestContainer defaultOpened withArrow={false} />)
    expect(container.querySelectorAll('.rviseui-Popover-arrow')).toHaveLength(0)
  })
