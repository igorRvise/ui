...
export function itSupportsInputWrapperProps<P>(Component: React.ComponentType<P>, requiredProps: P, name: string) {
  itSupportsWrapperProps(Component, requiredProps)

  it('handles required attribute correctly', () => {
    const { container } = render(<Component {...requiredProps} required id="secret-test-id" label="Test label" />)
    const requiredGlyph = container.querySelector(`.rviseui-${name}-required`)
  ...
  it('supports labelProps', () => {
    render(<Component {...requiredProps} label="Test label" labelProps={{ 'data-test': 'test-data' }} />)
  ...
  it('supports errorProps', () => {
    render(
      <Component {...requiredProps} error="Test error" errorProps={{ id: 'test-error', 'data-test': 'test-data' }} />,
    )
