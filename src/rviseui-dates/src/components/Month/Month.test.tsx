...
describe('@rviseui/dates/Month', () => {
  itSupportsSystemProps({
    component: Month,
    props: defaultProps,
    displayName: '@rviseui/dates/Month',
    refType: HTMLTableElement,
  })

  it('renders correct amount of weekdays', () => {
    const { container } = render(<Month month={new Date()} />)
    expect(container.querySelectorAll('.rviseui-Month-weekdayCell')).toHaveLength(7)
  })
  ...
  it('renders correct amount of days', () => {
    ...
    const { container: firstDayOfWeekSunday } = render(<Month month={new Date(2021, 1, 2)} firstDayOfWeek="sunday" />)
  ...
  it('assigns values to given daysRefs', () => {
    ...
    expect(daysRefs.every((list) => list.every((date) => date instanceof HTMLButtonElement))).toBe(true)
  })
  ...
  it('adds styles when month days are within range', () => {
    ...
    const { container: withRange } = render(
      <Month month={new Date(2021, 11, 1)} range={[new Date(2021, 11, 5), new Date(2021, 11, 15)]} />,
    )
  ...
  it('does not add styles when month days outside of range', () => {
    const { container } = render(
      <Month month={new Date(2021, 11, 1)} range={[new Date(2021, 10, 5), new Date(2021, 10, 15)]} />,
    )
  ...
  it('adds partial range styles', () => {
    const { container } = render(
      <Month month={new Date(2021, 11, 1)} range={[new Date(2021, 11, 25), new Date(2021, 12, 15)]} />,
    )
  ...
  it('changes first day of week based on prop', () => {
    const { container: sunday } = render(<Month {...defaultProps} firstDayOfWeek="sunday" locale="en" />)
    const { container: monday } = render(<Month {...defaultProps} firstDayOfWeek="monday" locale="en" />)

    expect(sunday.querySelectorAll('.rviseui-Month-weekdayCell')[0].textContent).toBe('Su')
    expect(sunday.querySelectorAll('.rviseui-Month-weekdayCell')[6].textContent).toBe('Sa')

    expect(monday.querySelectorAll('.rviseui-Month-weekdayCell')[0].textContent).toBe('Mo')
    expect(monday.querySelectorAll('.rviseui-Month-weekdayCell')[6].textContent).toBe('Su')
  })
  ...
  it('renders weekday labels with custom format in given locale', () => {
    ...
    const { container: ru } = render(<Month {...defaultProps} locale="ru" weekdayLabelFormat="dddd" />)
