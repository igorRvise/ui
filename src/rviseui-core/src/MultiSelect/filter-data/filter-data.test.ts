...
describe('@rviseui/core/MultiSelect/filter-data', () => {
  it('returns unmodified data if input is not searchable and value is empty', () => {
    expect(filterData({ ...baseOptions, searchable: false, searchValue: '', value: [] })).toBe(data)
  })

  it('removes selected items from input that is not searchable', () => {
    expect(filterData({ ...baseOptions, searchable: false, searchValue: '', value: ['vue', 'ng'] })).toStrictEqual(
      data.filter((item) => item.value !== 'vue' && item.value !== 'ng'),
    )
  })

  it('filters items with given filter function', () => {
    const filter = (searchValue: string, selected: boolean, item: SelectItem) => item.name.includes(searchValue)
    expect(
      filterData({
