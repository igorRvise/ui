...
;(async () => {
  ...
  if (!argv.skipPublish) {
    ...
    await Promise.all(
      packages.map((p) => {
        console.log(p.packageJson.name.includes('storybook'))
        console.log(p.packageJson.name)
        if (p.packageJson.name.includes('storybook') && p.packageJson.name.includes('tests')) {
          return false
        }
        return publishPackage({ path: p.path, name: p.packageJson.name, tag: argv.tag })
      }),
    )
