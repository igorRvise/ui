...
describe('@rviseui/dates/get-month-days', () => {
  it('returns all month days for given date', () => {
    ...
    expect(monthDays[monthDays.length - 1][monthDays[0].length - 1]).toStrictEqual(new Date(2021, 2, 0))
  })
  ...
  it('returns outside days for given month', () => {
    ...
    expect(monthDays[monthDays.length - 1][monthDays[0].length - 1]).toStrictEqual(new Date(2021, 4, 2))
  })
  it('returns outside days for given month, first day of the week - sunday', () => {
    ...
    expect(monthDays[monthDays.length - 1][monthDays[0].length - 1]).toStrictEqual(new Date(2021, 4, 1))
  })
})
