...
export function usePopover(options: UsePopoverOptions) {
  ...
  const { getReferenceProps, getFloatingProps } = useInteractions([
    useClick(floating.context, { enabled: options.trigger === 'click' && typeof options.opened !== 'boolean' }),
    useHover(floating.context, {
      enabled: options.trigger === 'hover',
      handleClose: safePolygon(),
      delay: { open: options.openDelay, close: options.closeDelay },
    }),
    useFocus(floating.context, { enabled: options.trigger === 'focus', keyboardOnly: true }),
  ])
