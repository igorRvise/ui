...
import { useScrollLock, useFocusTrap, useFocusReturn, useId } from '@rviseui/hooks'
import {
  DefaultProps,
  RviseNumberSize,
  RviseShadow,
...
import { GroupedTransition, RviseTransition } from '../Transition'
...
export interface DrawerProps
  extends Omit<DefaultProps<DrawerStylesNames>, RviseStyleSystemSize>,
...
const transitions: Record<DrawerPosition, RviseTransition> = {
  top: 'slide-down',
  bottom: 'slide-up',
  left: 'slide-right',
  right: 'slide-left',
...
const rtlTransitions: Record<DrawerPosition, RviseTransition> = {
  top: 'slide-down',
  bottom: 'slide-up',
  right: 'slide-right',
  left: 'slide-left',
...
export function Drawer(props: DrawerProps) {
  ...
  const _overlayOpacity =
    typeof overlayOpacity === 'number' ? overlayOpacity : theme.colorScheme === 'dark' ? 0.85 : 0.75
  ...
  return (
    <OptionalPortal withinPortal={withinPortal} target={target}>
      <GroupedTransition
        ...
        onEntered={() => _lockScroll(lockScroll)}
        mounted={opened}
        ...
        {(transitionStyles) => {
          const shouldTrigger = (event) => {
            return event.target.getAttribute('data-rviseui-stop-propagation') !== 'true'
          }

          const handleKeyDownCapture = (event) => {
            if (shouldTrigger(event) && event.key === 'Escape' && closeOnEscape) {
              onClose()
            }
          }
...
Drawer.displayName = '@rviseui/core/Drawer'
