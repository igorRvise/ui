...
export function getIncrementedVersion(version: string, options: { type: string; stage?: string }): string {
  ...
  if (options.stage && !VERSION_STAGE.includes(options.stage)) {
    logger.error(
      `Incorrect version stage: ${chalk.cyan(options.stage)}, it should be one of these values: ${VERSION_STAGE.join(
        ', ',
      )}`,
    )
