...
const getRightSection = (container: HTMLElement) => container.querySelector('.rviseui-NumberInput-rightSection')

const clickIncrement = (container: HTMLElement) =>
  userEvent.click(container.querySelector('.rviseui-NumberInput-controlUp'))

const clickDecrement = (container: HTMLElement) =>
  userEvent.click(container.querySelector('.rviseui-NumberInput-controlDown'))
...
describe('@rviseui/core/NumberInput', () => {
  checkAccessibility([
  ...
  itSupportsSystemProps({
    ...
    props: defaultProps,
    displayName: '@rviseui/core/NumberInput',
  ...
  it('does not increment or decrements out of min and max', async () => {
    const spy = jest.fn()
    const { container } = render(<NumberInput value={5} max={10} min={0} step={6} onChange={spy} />)
    await clickIncrement(container)
    expect(spy).toHaveBeenLastCalledWith(10)
    await clickDecrement(container)
    await clickDecrement(container)
  ...
  it('supports removing trailing zero with increment/decrement', async () => {
    ...
    const { container } = render(
      <NumberInput defaultValue={0.05} removeTrailingZeros precision={2} min={-1} step={0.05} max={1} onChange={spy} />,
    )
    await clickIncrement(container)
    expectValue('0.1')
