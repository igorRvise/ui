...
describe('@rviseui/core/Modal', () => {
  ...
  it('allows to hide close button', () => {
    ...
    const { container: withoutButton } = render(<Modal {...defaultProps} withCloseButton={false} />)
    expect(withButton.querySelectorAll('.rviseui-Modal-close')).toHaveLength(1)
    expect(withoutButton.querySelectorAll('.rviseui-Modal-close')).toHaveLength(0)
  })
  ...
  it('does not render modal header if close button is hidden and title is not passed', () => {
    ...
    const { container: withoutHeader } = render(<Modal {...defaultProps} title={null} withCloseButton={false} />)
    expect(withHeader.querySelectorAll('.rviseui-Modal-header')).toHaveLength(1)
    expect(withoutHeader.querySelectorAll('.rviseui-Modal-header')).toHaveLength(0)
  })

  it('has correct displayName', () => {
    expect(Modal.displayName).toStrictEqual('@rviseui/core/Modal')
  })
})
