...
describe('@rviseui/core/Select/filter-data', () => {
  ...
  it('does not filter data if search value is an exact match with selected item label and filterDataOnExactSearchMatch is set to false', () => {
    expect(filterData({ ...baseOptions, searchable: true, searchValue: 'React', value: 'react' })).toStrictEqual(data)

    expect(filterData({ ...baseOptions, searchable: true, searchValue: 'Reac', value: 'react' })).toStrictEqual([
      { value: 'react', label: 'React' },
    ])
