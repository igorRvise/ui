...
describe('@rviseui/dates/Day/get-day-props', () => {
  ...
  it('detects outside date', () => {
    expect(getDayProps({ ...defaultProps, date: new Date(2021, 11, 9), month: new Date(2021, 11, 1) })).toStrictEqual({
      ...defaultResult,
      outside: false,
    })

    expect(getDayProps({ ...defaultProps, date: new Date(2021, 10, 9), month: new Date(2021, 11, 1) })).toStrictEqual({
      ...defaultResult,
      outside: true,
    })
  })
  ...
  it('detects disabled date', () => {
    ...
    expect(getDayProps({ ...defaultProps, minDate: new Date(2040, 1, 1) })).toStrictEqual(disabledResults)
    expect(getDayProps({ ...defaultProps, maxDate: new Date(2000, 1, 1) })).toStrictEqual(disabledResults)
    expect(getDayProps({ ...defaultProps, excludeDate: () => true })).toStrictEqual(disabledResults)
    expect(
      getDayProps({
        ...defaultProps,
