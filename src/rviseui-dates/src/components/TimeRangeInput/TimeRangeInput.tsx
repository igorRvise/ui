...
export const TimeRangeInput = forwardRef<HTMLInputElement, TimeRangeInputProps>((props: TimeRangeInputProps, ref) => {
  const {
    required,
    label,
    error,
  ...
  useDidUpdate(() => {
    if (value[0]?.getTime() !== _value[0]?.getTime() || value[1]?.getTime() !== _value[1]?.getTime()) {
      setValue(value)
    }
  }, [value])
...
TimeRangeInput.displayName = '@rviseui/dates/TimeRangeInput'
