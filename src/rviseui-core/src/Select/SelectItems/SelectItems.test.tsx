...
describe('@rviseui/core/Select/SelectItems', () => {
  it('renders nothing found message if data array is empty', () => {
    ...
    const { container: withoutData } = render(<SelectItems {...defaultProps} data={[]} nothingFound="test-nothing" />)

    expect(withData.querySelectorAll('.rviseui-Select-nothingFound')).toHaveLength(0)
    expect(withoutData.querySelector('.rviseui-Select-nothingFound').textContent).toBe('test-nothing')
  })
  ...
  it('registers refs of all items', () => {
    ...
    expect(Object.keys(refs.current).every((ref) => refs.current[ref] instanceof HTMLDivElement)).toBe(true)
  })

  it('has correct displayName', () => {
    expect(SelectItems.displayName).toStrictEqual('@rviseui/core/SelectItems')
  })
})
