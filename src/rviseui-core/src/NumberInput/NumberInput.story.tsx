...
import { RVISE_SIZES } from '@rviseui/styles'
import { storiesOf } from '@storybook/react'

import { NumberInput } from './NumberInput'

const sizes = RVISE_SIZES.map((size) => <NumberInput defaultValue={0} label={size} size={size} key={size} mt="xl" />)

function Controlled(props: Omit<React.ComponentPropsWithoutRef<typeof NumberInput>, 'value' | 'onChange'>) {
...
storiesOf('NumberInput', module)
  ...
  .add('Step On Hold', () => (
    <>
      ...
      <div style={{ padding: 40, maxWidth: 400 }}>
        <NumberInput label="With max value" min={0} max={10} stepHoldDelay={500} stepHoldInterval={100} />
      </div>
      <div style={{ padding: 40, maxWidth: 400 }}>
        <NumberInput
