import { createStyles, RviseNumberSize, RviseColor, RviseSize } from '@rviseui/styles'
import { iconSizes } from './Step/Step.styles'

export interface StepperStylesParams {
  contentPadding: RviseNumberSize
  iconSize?: number
  size: RviseSize
  color: RviseColor
  orientation: 'vertical' | 'horizontal'
  iconPosition: 'right' | 'left'
  breakpoint: RviseNumberSize
}

export default createStyles(
  (theme, { contentPadding, color, orientation, iconPosition, iconSize, size, breakpoint }: StepperStylesParams) => {
    ...
    const separatorOffset =
      typeof iconSize !== 'undefined' ? iconSize / 2 - 1 : theme.fn.size({ size, sizes: iconSizes }) / 2 - 1
