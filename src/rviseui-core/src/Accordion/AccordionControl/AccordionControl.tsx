...
import { DefaultProps, Selectors, useComponentDefaultProps } from '@rviseui/styles'
import { createScopedKeydownHandler } from '@rviseui/utils'
...
export const AccordionControl = forwardRef<HTMLButtonElement, AccordionControlProps>(
  (props: AccordionControlProps, ref) => {
    const { disabled, onKeyDown, onClick, chevron, children, className, icon, ...others } = useComponentDefaultProps(
      'AccordionControl',
      defaultProps,
      props,
    )
    ...
    const content = (
      <UnstyledButton<'button'>
        ...
        <div className={classes.chevron} data-rotate={(!ctx.disableChevronRotation && isActive) || undefined}>
          {chevron || ctx.chevron}
        </div>
...
AccordionControl.displayName = '@rviseui/core/AccordionControl'
