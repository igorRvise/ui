...
describe('@rviseui/core/MultiSelect', () => {
  ...
  itSupportsSystemProps({
    ...
    props: defaultProps,
    displayName: '@rviseui/core/MultiSelect',
  ...
  it('Has un-filtered list when disableSelectedItemFiltering is enabled', () => {
    const { container } = render(<MultiSelect {...defaultProps} initiallyOpened disableSelectedItemFiltering />)

    expect(container.querySelectorAll('.rviseui-MultiSelect-item')).toHaveLength(4)
  })
  ...
  it('Has un-filtered list when disableSelectedItemFiltering is disabled', () => {
    ...
    expect(container.querySelectorAll('.rviseui-MultiSelect-item')).toHaveLength(2)
  })
})
