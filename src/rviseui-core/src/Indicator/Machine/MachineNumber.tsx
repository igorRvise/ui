...
import { usePrevious } from '@rviseui/hooks'
...
export const MachineNumber = forwardRef<HTMLDivElement, MachineNumberProps>((props, ref) => {
  ...
  const oldNumberScrollAnimationClass = useMemo(
    () =>
      isActive ? (scrollAnimationDirection === 'up' ? classes.oldNumberScrollUp : classes.oldNumberScrollDown) : null,
    [isActive, scrollAnimationDirection],
  )
  return (
    <span ref={ref} className={classes.baseNumber}>
      {(oldNumber && (
        <span className={cx(classes.oldNumber, classes.currentNumberTop, oldNumberScrollAnimationClass)}>
          {oldNumber}
        </span>
      )) ||
        null}
      <span>
        <span className={cx(classes.currentNumber, newNumberScrollAnimationClass)}>{newNumber}</span>
      </span>
      {(oldNumber && (
        <span className={cx(classes.oldNumber, classes.oldNumberBottom, oldNumberScrollAnimationClass)}>
          {oldNumber}
        </span>
      )) ||
        null}
