...
async function writeVersionToPackageJson(filePath: string, version: string) {
  ...
  if (current.peerDependencies) {
    Object.keys(current.peerDependencies).forEach((packageName) => {
      if (packageName.includes('@rviseui/')) {
...
export async function setPackagesVersion(version: string) {
  ...
  const folders = (await fs.readdir(src)).filter((folder) => fs.pathExistsSync(path.join(src, folder, '/package.json')))

  await Promise.all(
    folders.map((folder) => writeVersionToPackageJson(path.join(src, folder, '/package.json'), version)),
  )
