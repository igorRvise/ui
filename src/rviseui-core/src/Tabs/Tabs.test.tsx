...
import { itSupportsSystemProps, itRendersChildren, checkAccessibility } from '@rviseui/tests'
...
describe('@rviseui/core/Tabs', () => {
  ...
  it('unmounts Tab.Panel content if keepMounted is set to false', () => {
    const { container } = render(<Tabs {...defaultProps} keepMounted={false} defaultValue="tab-1" />)
    expect(container.querySelectorAll('.rviseui-Tabs-panel')[1].textContent).toBe('')
  })
