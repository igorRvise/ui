...
describe('@rviseui/hooks/use-list-state', () => {
  ...
  it('inserts item at given position with handlers.insert', () => {
    ...
    expect(multipleState).toStrictEqual([TEST_STATE[0], TEST_ITEM_1, TEST_ITEM_2, TEST_STATE[1], TEST_STATE[2]])
  })
  ...
  it('applies given function to respective item with handlers.applyWhere', () => {
    ...
    const [state] = hook.result.current
    expect(state).toStrictEqual([TEST_STATE[0], TEST_STATE[1], { ...TEST_STATE[2], experienced: true }])
  })
