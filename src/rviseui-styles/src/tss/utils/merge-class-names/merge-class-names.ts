...
import { useRviseProviderStyles } from '../../../theme/RviseProvider'

interface Input<T extends Record<string, string>> {
  cx(...classNames: any): string
  classes: T
  context: ReturnType<typeof useRviseProviderStyles>
  ...
  return Object.keys(classes).reduce((acc, className) => {
    acc[className] = cx(
      ...
      Array.isArray(name)
        ? name
            .filter(Boolean)
            .map((part) => `${cache?.key || 'rvise'}-${part}-${className}`)
            .join(' ')
        : name
        ? `${cache?.key || 'rvise'}-${name}-${className}`
        : null,
    )
    return acc
  }, {}) as T
