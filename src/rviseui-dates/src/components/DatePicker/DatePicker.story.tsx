...
import { Button, Modal, RVISE_SIZES, RviseProvider, TextInput, Tooltip } from '@rviseui/core'
import { useDisclosure } from '@rviseui/hooks'
import { SubmitForm, WithinOverlays } from '@rviseui/storybook'
...
const sizes = RVISE_SIZES.map((size) => (
  <DatePicker
...
storiesOf('DatePicker', module)
  ...
  .add('Date format on RviseProvider', () => (
    <RviseProvider theme={{ dateFormat: 'MM YYYY DD' }}>
  ...
  .add('Within form: free input', () => (
    <SubmitForm>
      <DatePicker placeholder="Submit with enter with free input" label="Event date" allowFreeInput />
    </SubmitForm>
  ))
  ...
  .add('Weekend days', () => (
    <div style={{ padding: 40, maxWidth: 400 }}>
      <DatePicker placeholder="Submit with enter" label="Event date" weekendDays={[1, 3]} />
    </div>
  ))
