...
describe('@rviseui/core/Drawer', () => {
  ...
  it('allows to hide close button with withCloseButton={false} prop', () => {
    ...
    const { container: withoutCloseButton } = render(<Drawer {...defaultProps} withCloseButton={false} />)

    expect(withoutCloseButton.querySelectorAll('.rviseui-Drawer-closeButton')).toHaveLength(0)
    expect(withCloseButton.querySelectorAll('.rviseui-Drawer-closeButton')).toHaveLength(1)
  })

  it('has correct displayName', () => {
    expect(Drawer.displayName).toStrictEqual('@rviseui/core/Drawer')
  })
})
