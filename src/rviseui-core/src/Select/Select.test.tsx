...
import { itSupportsFocusEvents, checkAccessibility, itSupportsSystemProps, itSupportsInputProps } from '@rviseui/tests'
...
describe('@rviseui/core/Select', () => {
  ...
  itSupportsSystemProps({
    component: Select,
    props: defaultProps,
    displayName: '@rviseui/core/Select',
    refType: HTMLInputElement,
    othersSelector: '.rviseui-Select-input',
    providerName: 'Select',
  })

  it('renders hidden input with current input value', () => {
    const { container } = render(<Select {...defaultProps} name="custom-select" form="custom-form" value="test-1" />)
    expect(container.querySelector('input[name="custom-select"]')).toHaveValue('test-1')
    expect(container.querySelector('input[name="custom-select"]')).toHaveAttribute('form', 'custom-form')
  })
