...
export function itSupportsInputProps<P>(Component: React.ComponentType<P>, requiredProps: P, name: string) {
  ...
  it('handles error and invalid state', () => {
    const { container: invalid } = render(<Component {...requiredProps} required id="invalid-test-id" error />)
    ...
    expect(invalid.querySelectorAll(`.rviseui-${name}-error`)).toHaveLength(0)
    expect(withError.querySelector(`.rviseui-${name}-error`)).toBeInTheDocument()
    expect(withError.querySelector(`.rviseui-${name}-error`).textContent).toBe('Test error')
  })
