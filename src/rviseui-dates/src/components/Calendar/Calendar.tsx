...
import { useComponentDefaultProps } from '@rviseui/core'
...
export interface CalendarProps<Multiple extends boolean = false>
  extends Omit<CalendarBaseProps, 'value' | 'onChange' | 'isDateInRange' | 'isDateFirstInRange' | 'isDateLastInRange'> {
  multiple?: Multiple
...
export function Calendar<Multiple extends boolean = false>(props: CalendarProps<Multiple>) {
  ...
  const handleChange = (date: Date) => {
    ...
    return onChange(isSelected ? value.filter((val: Date) => !isSameDate(val, date)) : [...value, date])
  }

  return <CalendarBase __staticSelector={__staticSelector} onChange={handleChange} value={value} {...others} />
}

Calendar.displayName = '@rviseui/dates/Calendar'
