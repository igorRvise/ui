...
import { useUncontrolled, useDidUpdate, useEyeDropper } from '@rviseui/hooks'
import { DefaultProps, getDefaultZIndex, RviseShadow, useRviseTheme } from '@rviseui/styles'
import { noop } from '@rviseui/utils'
import {
...
export const ColorInput = forwardRef<HTMLInputElement, ColorInputProps>((props, ref) => {
  ...
  const eyeDropper = (
    <ActionIcon
      ...
      {eyeDropperIcon || <EyeDropperIcon size={theme.fn.size({ size: inputProps.size, sizes: EYE_DROPPER_SIZES })} />}
    </ActionIcon>
  )
        ...
        <Popover.Target>
          <div>
            <Input<'input'>
              ...
              styles={styles}
              rightSection={
                rightSection || (withEyeDropper && !disabled && !readOnly && eyeDropperSupported ? eyeDropper : null)
              }
              rightSectionWidth={
                rightSectionWidth ?? theme.fn.size({ size: inputProps.size, sizes: RIGHT_SECTION_WIDTH })
              }
            />
          </div>
        </Popover.Target>
...
ColorInput.displayName = '@rviseui/core/ColorInput'
