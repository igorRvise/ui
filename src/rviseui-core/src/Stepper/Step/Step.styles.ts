import { createStyles, RviseColor, RviseNumberSize, RviseSize } from '@rviseui/styles'
...
export default createStyles(
  (theme, { color, iconSize, size, radius, allowStepClick, iconPosition, orientation }: StepStylesParams, getRef) => {
    ...
    const separatorDistanceFromIcon = (theme.spacing.xs as number) / 2
    ...
    return {
      ...
      stepBody: {
        ...
        ...(orientation === 'vertical'
          ? {
              marginTop:
                _iconSize > theme.fn.size({ size, sizes: theme.fontSizes }) * 4 ? _iconSize / 4 : _iconSize / 12,
            }
          : null),
      },
