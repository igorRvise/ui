      ...
      if (node) {
        const processNode = () => {
          ...
          if (focusElement) {
            ...
            console.warn('[@rviseui/hooks/use-focus-trap] Failed to find focusable element within provided node', node)
          }
        }
        ...
        setTimeout(() => {
          if (node.getRootNode()) {
            ...
            // eslint-disable-next-line no-console
            console.warn('[@rviseui/hooks/use-focus-trap] Ref node is not part of the dom', node)
          }
        })
