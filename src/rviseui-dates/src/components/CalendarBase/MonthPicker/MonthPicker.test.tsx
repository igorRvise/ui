...
import { itSupportsClassName, checkAccessibility } from '@rviseui/tests'
import userEvent from '@testing-library/user-event'
import { MonthPicker, MonthPickerProps } from './MonthPicker'

const CONTROL_SELECTOR = '.rviseui-MonthPicker-monthPickerControl'
const ACTIVE_CONTROL_SELECTOR = '.rviseui-MonthPicker-monthPickerControlActive'
const LEVEL_SELECTOR = '.rviseui-MonthPicker-calendarHeaderLevel'
const HEADER_CONTROL_SELECTOR = '.rviseui-MonthPicker-calendarHeaderControl'
...
describe('@rviseui/core/MonthPicker', () => {
  itSupportsClassName(MonthPicker, defaultProps)
  checkAccessibility([<MonthPicker {...defaultProps} nextYearLabel="Next" previousYearLabel="Previous" />])
  ...
  it('renders correct amount of controls', () => {
    ...
    const { container: disabled } = render(
      <MonthPicker {...defaultProps} minDate={new Date(2021, 1, 1)} maxDate={new Date(2021, 5, 1)} />,
    )
  ...
  it('adds active styles to selected month', () => {
    const { container } = render(<MonthPicker {...defaultProps} value={{ year: 2021, month: 11 }} />)
  ...
  it('sets hasNext/hasPrevious CalendarHeader props based on minDate/maxDate props', () => {
    const { container: withMaxYear } = render(<MonthPicker {...defaultProps} maxDate={new Date(2021, 11, 1)} />)
    const { container: withMinYear } = render(<MonthPicker {...defaultProps} minDate={new Date(2021, 11, 1)} />)
  ...
  it('passes nextLabel/previousLabel props to CalendarHeader component', () => {
    render(<MonthPicker {...defaultProps} nextYearLabel="next-test" previousYearLabel="previous-test" />)
  ...
  it('has correct displayName', () => {
    expect(MonthPicker.displayName).toStrictEqual('@rviseui/dates/MonthPicker')
  })
})
